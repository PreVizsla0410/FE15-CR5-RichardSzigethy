...
class Cat extends Animal {
    ...
    displayCard() {
        ...
        <div class="card-body">
         ...
         <h6 class="cart-title">${this.breedInfo}</h6>
        </div>
      </div>
      </div>
      `
...
function displayAnimals(animals: Array < Animal > , sorting ? : sortAge): void { //void - this function won´t return anything
    const row = (document.getElementById("row") as HTMLElement); //sorting? is optional
    let animalsArray = [...animals]; //clean copy to animals array
    if (sorting) { //if sorting is existin - it is true - else it is false
        if (sorting === sortAge.ascending) {
            animalsArray = animals.sort((a, b) => b.age - a.age); //b.age is the last value (a,b) so we go from the biggest to the lowest
        } else {
            animalsArray = animals.sort((a, b) => a.age - b.age); //vice versa
        }
    }
    row.innerHTML = animalsArray.map(animal => animal.displayCard()).join(''); //map is an arrayfunction converts each elment of the array into something
}
...
let ageOrder = sortAge.ascending;


const animals: Array < Animal > = [animal1, animal2, animal3, animal4, animal5, animal6];
displayAnimals(animals);

const toggleAgeBtn = document.getElementById('btn-age-toggle');
